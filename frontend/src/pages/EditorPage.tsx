...
const EditorPage = () => {
  ...
  const handleChapterChange = (e: any) => {
    ...
    setBook((prev: any) => ({ ...prev, chapters: updatedChapters }));
  };
  ...
  const handleSaveChanges = async (bookToSave: any, showToast: boolean) => {
    setIsSaving(true);
    try {
      await axiosInstance.put(
        `${API_PATH.BOOK.UPDATE_BOOK}/${bookId}`,
        bookToSave
      );
  ...
  const handleCoverImageUpload = async (
    ...
    try {
      const response = await axiosInstance.put(
        `${API_PATH.BOOK.UPDATE_BOOK_COVER}/${bookId}`,
        formData,
        {
          headers: {
  ...
  const handleGenerateChapterContent = async (index: any) => {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `${API_PATH.EXPORT.PDF}/${bookId}/pdf`,
        { responseType: "blob" }
      );
  ...
  const handleExportDoc = async () => {
    ...
    try {
      const response = await axiosInstance.get(
        `${API_PATH.EXPORT.DOC}/${bookId}/doc`,
        { responseType: "blob" }
      );
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="hidden sm:flex space-x-1 bg-slate-100 p-1 rounded-lg">
                <button
                  ...
                  className={`flex items-center justify-center flex-1 py-2 px-4 text-sm font-medium rounded-md transition-colors duartion-200 ${
                    activeTab === "editor"
                ...
                <button
                  ...
                  className={`flex items-center justify-center flex-1 py-2 px-4 text-sm font-medium rounded-md transition-colors duartion-200 whitespace-nowrap ${
                    activeTab === "details"
                      ? "bg-white text-slate-800 shadow-sm"
                      : "text-slate-500 hover:text-slate-700"
                  } `}
