...
const processMarkdownToDocx = (markdown) => {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    try {
      if (token.type === "heading_open") {
        ...
        if (nextToken && nextToken.type === "paragraph_open") {
          ...
          if (
            ...
            i += 4; // Skip paragraph_open, inline, paragraph_close, list_items_close
          }
        }
      ...
      console.log("Error processing token: ", token.type, error);
    }
  }
...
const exportAsDocument = async (req, res) => {
  try {
    ...
    // Generating the document buffer
...
// Typorgraphy configuration for ebook styling
const TYPOGRAPHY = {
  fonts: {
    serif: "Times-Roman",
...
const renderInlineTokens = (doc, tokens, options = {}) => {
  ...
  if (textBuffer) {
    doc
      .font(currentFont)
      .text(textBuffer, { ...baseOptions, continued: false });
  } else {
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    try {
      ...
      if (token.type === "heading_open") {
        ...
        // consume the heading_close if present
      ...
      if (token.type === "paragraph_open") {
        ...
        // consume paragraph_close if present
      ...
      if (token.type === "list_item_open") {
        ...
        // print bullet and keep continued true so inline content appears on same line
        doc.text(bullet, { indent: 20, continued: true });

        // find the inline token(s) that belong to this list item
        let consumedIndex = i;
        for (let j = i + 1; j < tokens.length; j++) {
          const t = tokens[j];

          if (t.type === "inline" && t.children) {
            renderInlineTokens(doc, t.children, {
              align: "left",
              lineGap: 2,
              continued: false,
            });
        ...
        // ensure the line is finished
        if (!doc._.ended && doc._writing) {
          // if continued was used above, it should already have finished; otherwise move down.
        }
        doc.moveDown(TYPOGRAPHY.spacing.listSpacing / TYPOGRAPHY.sizes.body);

        // advance outer loop to the token we consumed
        i = consumedIndex;
        continue;
      }
      ...
      if (token.type === "code_block" || token.type === "fence") {
        ...
        doc
          .font("Courier") // standard PDF font
          .fontSize(9)
          .fillColor(TYPOGRAPHY.colors.text)
      ...
      // fallback: ignore token types you don't handle
    } catch (error) {
      console.log("Error processing token: ", token && token.type, error);
